...
# 요구사항 
### ServerSocket 에 관하여
* 실습에 들어가기 앞서 TCP/IP 통신에 대해서 알아야 합니다.   
  TCP/IP 통신은 소켓을 통한 통신으로 Server Socket 과 Client Socket이 존재합니다.
...
<br/>
클라이언트의 연결 요청이 수신되면 accept 메소드는 그 즉시 소켓 객체를 리턴합니다. 서버 소켓이 클라이언트와 통신하기 위한 소켓을 리턴합니다.  

그러면 coonection 변수에 소켓이 세팅되면서 connection 변수가 null 이 아니게 되니 while 문이 실행됩니다.  
<br/>
